...
def genera_ricevuta_completa(d):
    ...
    p.drawString(50, h - 150, f"Telefono: {d['telefono']}")
    p.drawString(50, h - 165, f"Destinazione: {d['destinazione']}")
    ...
    p.drawString(60, h - 220, f"Totale Lordo: {d['totale_lordo']:.2f}€ | Anticipo: {d['acconto']:.2f}€")
    ...
    p.setFont("Helvetica", 9)
    note_obj = p.beginText(60, 170)
    note_obj.textLines(d['note'])
    p.drawText(note_obj)
...
if dest_addr:
    try:
        ...
        st.divider()
        conferma = st.checkbox("✅ Procedi con i dati del Prodotto e Modulo Consegna")
        ...
        if conferma:
            ...
            if tipo_pag == "Saldato in Negozio":
                acconto, saldo_finale = totale_lordo, 0.0
            else:
            ...
            d_pdf = {
                ...
                "note": note_libere if note_libere else ""
            }

            st.download_button("🖨️ STAMPA MODULO CONSEGNA", data=genera_ricevuta_completa(d_pdf), file_name=f"Yammo_{cognome_cl}.pdf", mime="application/pdf", use_container_width=True)
                
    except Exception as e:
        st.error(f"Errore tecnico: {e}")
else:
    st.info("💡 Inserisci l'indirizzo per iniziare.")
